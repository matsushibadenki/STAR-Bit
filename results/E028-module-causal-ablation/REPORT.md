...
rotate2の成功解は1件だけで、不偏分散は定義できない。実行時の非標準JSON `NaN` を元ファイル `results.original.json` に保存し、正規化版 `results.json` では該当箇所だけ `null` にした。[正規化記録](run/normalization.json)。個別探索recordは変更していない。
...
[事前計画](PROTOCOL.md) / [生データ](run/results.json) / [凍結Library](run/frozen_library.json) / [監査要約](run/audit_summary.json)
