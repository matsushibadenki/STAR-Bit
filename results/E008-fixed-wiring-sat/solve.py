...
def proof_dag(proof):
    """Keep sharing: sexpr() can exponentially expand a proof DAG."""
    start=time.monotonic();stack=[proof];seen={};complete=True;reason=None
    while stack:
        if len(seen)>=100000 or time.monotonic()-start>3:
            complete=False;reason='proof_export_budget';break
        node=stack.pop();key=node.get_id()
        if key in seen:continue
        if not z3.is_app(node):
            complete=False;reason='unsupported_proof_ast';break
        children=node.children();params=[]
        for param in node.decl().params():
            if isinstance(param,z3.AstRef):
                params.append({'ast_id':param.get_id()});stack.append(param)
            else:params.append(str(param))
        seen[key]=dict(kind=node.decl().kind(),name=str(node.decl().name()),sort=str(node.sort()),
                       children=[c.get_id() for c in children],params=params)
        stack.extend(children)
    return dict(root_id=proof.get_id(),nodes=seen,complete=complete,reason=reason,format='Z3 AST DAG, not independently proof-checked')

def run():
    root=Path(__file__).resolve().parent;out=root/'run_bounded';out.mkdir(exist_ok=False)
        ...
        for task in ('numeric','selection'):
            ...
            for mode in ('gate2','lut4'):
                ...
                if status==z3.sat:
                    ...
                    proof=proof_dag(solver.proof())
                    with gzip.open(out/f'{name}-proof-dag.json.gz','wt') as file:json.dump(proof,file)
                    record['proof_export_complete']=proof['complete']
                    record['proof_export_reason']=proof['reason']
