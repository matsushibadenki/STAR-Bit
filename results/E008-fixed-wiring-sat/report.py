...
def main():
    ...
    extra=json.loads((root/'run/additional_checks.json').read_text())
    prior={a['name'] for a in extra['known_trained_witnesses']}
    counts={s:sum(a['status']==s for a in r['records']) for s in ('sat','unsat','unknown')}
    known=sum(a['status']=='sat' or f"{a['task']}-{a['seed']}-{a['condition']}" in prior for a in r['records'])
    failed=sum(a['status']=='sat' and a['learned_hard_exact']<1 for a in r['records'])
    partial=sum(a.get('proof_export_complete') is False for a in r['records'])
    lines=['# E008：固定配線で正解回路を構成できるか', '', '実行日：2026-09-11。', '',
           f"E007の64配線を診断した。ソルバー判定はSAT {counts['sat']}件、UNSAT {counts['unsat']}件、UNKNOWN {counts['unknown']}件。既存の学習済み完全一致モデルの証人も含め、構成可能性を確認した配線は{known}件。", '',
    ...
    for a in r['aggregates']:
        c=a['counts'];d=a['learned_accuracy']
        lines.append(f"| {a['task']} | {a['condition']} | {c['sat']} | {c['unsat']} | {c['unknown']} | {a['sat_with_learning_error']} | {d['mean']:.6f} | {d['variance']:.6f} | [{d['ci95'][0]:.6f}, {d['ci95'][1]:.6f}] |")
    lines+=['', '各行16seed。学習精度のCIはE007測定のseed bootstrapによる記述統計。ソルバー成功率と学習精度の有意差検定はしない。SATである配線だけ選んだ精度比較を全配線へ一般化しない。', '',
            f"ソルバーがSAT証人を得て、元の学習に誤差があったケースは{failed}件。この件数だけが今回、同配線での学習失敗を構成的に確認できた範囲である。UNKNOWNには最適化失敗とも表現不能とも割り当てない。", '',
            '## 既存の完全一致checkpoint', '',
    ...
    for row in r['records']:
        ...
        if name in prior:lines.append(f"| {name} | {row['status']} | 100% |")
    lines+=['', 'solverのUNKNOWN件数と、既存証人を加味した構成可能性未判定の件数は同じとは限らない。', '',
            '## 判定の意味と限界', '',
            ...
            f'UNSAT {counts["unsat"]}件のうち、証明DAGの保存上限に達したものは{partial}件。これらのproof artifactは不完全であり、完全な証明ファイルを納品したとは扱わない。DAG参照の整合性検査も証明の意味的な独立検証ではない。', '',
            '## 検証と再現', '',
            ...
            f"- Z3 {r['z3_version']}、NumPy {r['numpy']}、最終試行経過 {r['elapsed_seconds']:.3f}秒。proof有効時の10秒制限は多少超過し得るため、実測solve_secondsも記録。", '',
            '```bash', 'python3 -m pip install --target /private/tmp/starbit-z3 z3-solver==5.1.0.0', 'python3 results/E008-fixed-wiring-sat/solve.py', 'PYTHONPATH=/private/tmp/starbit-z3 python3 results/E008-fixed-wiring-sat/verify.py', 'python3 results/E008-fixed-wiring-sat/audit.py', 'python3 results/E008-fixed-wiring-sat/report.py', '```', '',
    ...
    (root.parents[1]/'docs/STAR-Bit-E008-fixed-wiring-sat.md').write_text(content)
