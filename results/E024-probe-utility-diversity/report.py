...
'## Roadmap','',
'- [Done] probe taskとevaluation taskを選定前に分離し、一段offspring utilityとsignature多様性を実装した。','- [Done] 4 seed、80探索、同費用random・frequency・no-transfer対照と平均・分散・CI・効果量・exact検定を保存した。','- [Done] 16-seed移行基準は不成立。全11 exact式を64入力で再検証した。','- [Done] E025でusage-gated evictionを比較したが、局所use判定が全Functionを通し、探索性能は回復しなかった。','- [Next] 一段lookaheadを止め、with/without Moduleのpaired rolloutによる反実仮想admissionを小規模に診断する。','- [Later] cross-family transfer成立後にRouterへ統合し、load balance、固定random route、Expert交換、総費用を再評価する。','',
