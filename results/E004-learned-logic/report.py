...
def main():
    ...
    lines=['# E004：学習した論理ゲート回路の抽出と温度annealing', '',
    ...
    for a in r['aggregates']:
        ...
        lines.append(f"| {a['task']} | {a['condition']} | {m['soft_test_exact']['mean']:.6f} | {d['mean']:.6f} | {d['variance']:.6f} | [{d['ci95'][0]:.6f}, {d['ci95'][1]:.6f}] | {m['hard_test_bit_accuracy']['mean']:.6f} | {m['hard_train_exact']['mean']:.6f} |")
    lines+=['', '## 主比較：annealed−fixedのhard test exact-match', '',
            '256通りの両側符号反転検定。2タスクを一つのfamilyとしてHolm補正。', '',
            '| タスク | 平均差 | 差の分散 | bootstrap 95% CI | 生p | Holm p |',
            '| --- | ---: | ---: | --- | ---: | ---: |']
    for a in r['contrasts']:
        lines.append(f"| {a['task']} | {a['mean']:.6f} | {a['variance']:.6f} | [{a['ci95'][0]:.6f}, {a['ci95'][1]:.6f}] | {a['p_signflip_two_sided']:.6f} | {a['p_holm']:.6f} |")
    lines+=['', '両タスクで平均差−1.171875 percentage points。改善仮説は支持されない。ただし悪化が確定したという結果でもない。selectionのbootstrap区間は0を含まないが、少数・同点を含むseed差で符号反転検定は有意でない。区間だけを選んで有意な悪化と報告しない。', '',
            '## Module抽出', '',
            '| タスク | 条件 | primitive記述平均 | 抽出後記述平均 | Module平均 | primitive DAG平均 |',
            '| --- | --- | ---: | ---: | ---: | ---: |']
    for a in r['aggregates']:
        c=a['compression']
        lines.append(f"| {a['task']} | {a['condition']} | {c['primitive_symbols']['mean']:.3f} | {c['compressed_symbols']['mean']:.3f} | {c['modules']['mean']:.3f} | {c['primitive_dag']['mean']:.3f} |")
    lines+=['', '全32runで、全64入力に対するhard網・primitive式・Module抽出後の出力が完全一致した。記述短縮は小さく、出力に寄与するprimitive回路も小さい。68個という割り当てゲート数と、出力へつながる展開DAGのサイズは別指標である。', '',
            '16種類のゲートをAND/OR/XOR/NOTの固定式へ展開するため、抽出したModuleはゲート符号化を再構成しているだけの場合もある。圧縮結果を抽象概念の獲得とみなさない。時間・面積・消費電力の改善は未測定。', '',
            '## 検証', '',
            f"- 16ゲートの全真理値表を独立に照合。解析勾配と中心差分の最大絶対誤差は {r['verification']['max_gradient_error']:.3g}（5パラメータを3段から検査）。",
            ...
            f"- NumPy {r['numpy']}。実行記録の経過時間 {r['elapsed_seconds']:.3f}秒。これは機器性能のbenchmark値として使わない。", '',
            '## 次の改善案', '',
    ...
    (root/'STAR-Bit-E004-learned-logic.md').write_text('\n'.join(lines))
